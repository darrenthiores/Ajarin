...
class GetMentorsPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Mentor> {
        return withContext(Dispatchers.IO) {
            ...
            Timber.d("ON GET MENTORS: $page")
            try {
                val response = remoteDataSource.getMentors(page)
                ...
                when (response) {
                    ...
                    is ApiResponse.Error -> {
                        throw CustomException("ON GET MENTORS ERROR: ${response.errorMessage}")
                    }
